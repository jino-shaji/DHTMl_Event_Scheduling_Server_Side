...
function checkStatus(){
    ...
    $("#salesDate").val(tcData[0].invoiceDate);
    $("#notes").val(tcData[0].notes);
    $("#salesInvoice").val(tcData[0].invoiceNumber);
    ...
    $("#selectState").val(tcData[0].state);
...
$("#buttonSave").on('click',function(){
  ...
  $.ajax({
            url: "stGetBasics",
            type: 'POST',
            data: {type:"editAdvance",iid:iid,Date:receiptDate,notes:notes,salesInvoice:salesInvoice,salesDate:salesDate,customerId:customerId,billingAddress:billingAddress,
            ...
            success: function (data, textStatus, jqXHR) {
                ...
                window.location="advanceInvoice.jsp?type=1&sid="+iid;
                $("#buttonSave").prop('disabled', false);
            },
          error: function (jqXHR, textStatus, errorThrown) {
                alert("Error");
