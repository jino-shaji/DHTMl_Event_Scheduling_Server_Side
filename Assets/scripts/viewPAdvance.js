...
function loadData(type){
 ...
 var tDate=$("#toDate").val();
    $.ajax({
        url: 'stGetBasics',
        type: 'GET',
        data: {type: 'getPAdvanceInvoice',iType:type,fromDate:fDate,toDate:tDate},
  ...
  // alert("d"+dat);
     ...
     table=         $('#tableInvoice').DataTable( {
        ...
        "columns": [
            ...
            { "data": "purchaseInvoice" ,"defaultContent": "" },
...
$(function(){
    ...
    $('#tableInvoice').on( 'click', '.view', function () {
        ...
        window.open("advanceInvoice.jsp?type=2&sid="+data["sid"]+"",'_blank');
    ...
    $('#tableInvoice').on( 'click', '.edit', function () {
        ...
        window.open("ePAdvance.jsp?iid="+data["sid"]+"",'_blank');
    ...
    $('#tableInvoice').on( 'click', '.cancel', function () {
        var dat = table.row( $(this).parents('tr') ).data();
         ...
         var c=confirm("Kindly Confirm ?");
            ...
            url: "stGetBasics",
            type: 'post',
            data: {iid:dat["sid"],type:"changeStatus",iType:"8"},
            success: function (data, textStatus, jqXHR) {
                if (data[0].status == "0") {
                    alert("Error");
                    console.log(data[0].errorMessage);
