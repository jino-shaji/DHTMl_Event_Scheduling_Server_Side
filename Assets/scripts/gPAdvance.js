...
$("#buttonSave").on('click',function(){
  ...
  $.ajax({
            url: "stPAdvance",
            type: 'POST',
            data: {advanceReceipt:advanceReceipt,receiptDate:receiptDate,notes:notes,customerId:customerId,purchaseInvoice:purchaseInvoice,invoiceDate:invoiceDate,items:JSON.stringify(jsonItem)},
            success: function (data, textStatus, jqXHR) {
                if (data[0].status == "0") {
                    alert("Error");
                    console.log(data[0].errorMessage);
                ...
                window.location="advanceInvoice.jsp?type=2&sid="+data[0].advanceId;
                $("#buttonSave").prop('disabled', false);
            },
          error: function (jqXHR, textStatus, errorThrown) {
                alert("Error");
