...
var statusC=0,statusI=0,statusB=0,statusTC;
var iid;    
...
$("#buttonSave").on('click',function(){
 ...
 // var reverse=$("#selectReverse").val();
  ...
  $.ajax({
            url: "stGetBasics",
            type: 'POST',
            data: {type:"editBOS",iid:iid,dueDate:dueDate,notes:notes,invoiceDate:invoiceDate,customerId:customerId,billingAddress:billingAddress,
                      shippingAddress:shippingAddress,stateId:pos,items:JSON.stringify(jsonItem)},
            success: function (data, textStatus, jqXHR) {
                console.log("data0 "+data[0].status);
                 if (data[0].status == "0") {
                    alert("Error");
                    console.log(data[0].errorMessage);
                    $("#buttonSave").prop('disabled', false);
                ...
                window.location="BOSInvoice.jsp?sid="+iid;
                $("#buttonSave").prop('disabled', false);
            },
          error: function (jqXHR, textStatus, errorThrown) {
                alert("Error");
