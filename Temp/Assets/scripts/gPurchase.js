...
$("#buttonSave").on('click',function(){
  ...
  $.ajax({
            url: "stPurchase",
            type: 'POST',
            data: {purchaseInvoice:purchaseInvoice,invoiceDate:invoiceDate,notes:notes,dueDate:dueDate,customerId:customerId,stateId:pos,reverse:reverse,items:JSON.stringify(jsonItem)},
            success: function (data, textStatus, jqXHR) {
                if (data[0].status == "0") {
                    alert("Error");
                    console.log(data[0].errorMessage);
                ...
                window.location="viewSInvoice.jsp?type=2&sid="+data[0].purchaseId;
                $("#buttonSave").prop('disabled', false);
            },
          error: function (jqXHR, textStatus, errorThrown) {
                alert("Error");
