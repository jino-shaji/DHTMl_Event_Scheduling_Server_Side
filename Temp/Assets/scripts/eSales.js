...
$("#buttonSave").on('click',function(){
  ...
  $.ajax({
            url: "stGetBasics",
            type: 'POST',
            data: {type:"editSales",iid:iid,Date:invoiceDate,notes:notes,dueDate:dueDate,customerId:customerId,billingAddress:billingAddress,
                      shippingAddress:shippingAddress,stateId:pos,reverse:reverse,items:JSON.stringify(jsonItem)},
            success: function (data, textStatus, jqXHR) {
                if (data[0].status == "0") {
                    alert("Error");
                    console.log(data[0].errorMessage);
                ...
                window.location="viewSInvoice.jsp?type=1&sid="+iid;
                $("#buttonSave").prop('disabled', false);
            },
          error: function (jqXHR, textStatus, errorThrown) {
                alert("Error");
